...
function load_style_script(){
    ...
    wp_enqueue_style('main', get_template_directory_uri() . '/css/fm_main.css',array(),rand(1,99) );
    wp_enqueue_style('fm_custom', get_template_directory_uri() . '/css/fm_custom.css',array(),rand(1,99) );


             
    wp_enqueue_script('jquery');
    wp_enqueue_script('fancybox', get_template_directory_uri() . '/js/jquery.fancybox.js');
    wp_enqueue_script('main', get_template_directory_uri() . '/js/fm_main.js',array('jquery'),rand(1,99));
}

add_action('wp_enqueue_scripts', 'load_style_script');

register_nav_menu('primary', 'Main Menu');
function fm_footer_bottom_left() {
    register_sidebar( array(
        'name' => __( 'Footer Нижняя левая', 'smallenvelop' ),
        'id' => 'fm_footer_bottom_left',

    ) );
}
add_action( 'widgets_init', 'fm_footer_bottom_left' );
function fm_footer_bottom_right() {
    register_sidebar( array(
        'name' => __( 'Footer правый нижний', 'smallenvelop' ),
        'id' => 'fm_footer_bottom_right',

    ) );
}
add_action( 'widgets_init', 'fm_footer_bottom_right' );
function fm_fm_menu_social() {
    register_sidebar( array(
        'name' => __( 'Menu Socials', 'smallenvelop' ),
        'id' => 'fm_fm_menu_social',

    ) );
}
add_action( 'widgets_init', 'fm_fm_menu_social' );
function fm_popup_form() {
    register_sidebar( array(
        'name' => __( 'Popup Form', 'smallenvelop' ),
        'id' => 'fm_popup_form',

    ) );
}
add_action( 'widgets_init', 'fm_popup_form' );
